...
- **Multiselect selection persists.** When a new store is added centrally, the user has to open the Stores multiselect and tick the new entry — the `default=ALL_STORES` is only applied on first render. A brand can therefore be correctly wired and STILL not show. Check this before debugging anything deeper.
- **A brand missing from the dashboard is almost never a code bug.** `ALL_STORES` is the intersection of the central file's `[shopify.*]` and `[printify.*]` keys, minus `EXCLUDED_STORES` — a brand with only one of the two sections is invisible, and nothing errors. Check in order: both sections exist centrally → key spelt identically in both → not in `EXCLUDED_STORES` → the multiselect gotcha above.
